...
Crie um banco de dados com o nome do seu projeto, lembre de utilizar um nome sem espaços ou caracteres especiais. Aqui vamos chamas nosso banco de `arcane_library;`.

```sql
CREATE DATABASE arcane_library;
```
...
```bash
 psql -h localhost  -d arcane_library -U arcane_library -W -f db/schema_create.sql
```
